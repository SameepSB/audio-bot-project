...
with st.sidebar:
    ...
    input_language = st.selectbox("Spoken Language", ["Auto-detect", "English", "Hindi", "Bengali"])
...
if audio_value:
    ...
    with st.status("👂 Listening and Transcribing...", expanded=True):
        ...
        transcript_response = openai.audio.transcriptions.create(
            ...
            **language_options.get(input_language, {})
        )
        user_text = transcript_response.text
        detected_language = language_options.get(input_language, {}).get(
        ...
        tts_language = tts_language_codes.get(
            str(detected_language).lower(), "en"
        )
    ...
    with st.status("🧠 Reasoning...", expanded=True):
        response = openai.chat.completions.create(
            model=model_choice,
            messages=[
                {"role": "system", "content": f"You are a professional assistant. Reply in the same language as the user ({detected_language}). Be concise."},
    ...
    with st.status("🗣️ Synthesizing Voice...", expanded=True):
        tts = gTTS(text=ai_text, lang=tts_language)
